...
println "- Unit-Test: Needs to check that there is a workdir for compiling  -";
println "-            set in the configuration file.                        -";
println "--------------------------------------------------------------------";
...
assert result.getErrorSections().contains(ConfigurationCheck.WORKDIR);
assert configuration.workdir == null;
